...
export class TransactionsTableComponent implements OnInit {
      ...
      .subscribe(
        (res: any) => {
          ...
          this.transactions = JSON.parse(
            JSON.stringify(res.data.myTransactions)
          ) as Transaction[];

          if (this.storeId) {
            this.transactions = this.transactions.filter(
              (t) =>
                t.store?._id === this.storeId || t.toStore?._id === this.storeId
            );
          }

          if (this.clientId) {
            this.transactions = this.transactions.filter(
              (t) => t.client?._id === this.clientId
            );
          }
          ...
          this.transactionsTable.dtInstance.then(
            (dtInstance: DataTables.Api) => {
              ...
              if (this.storeId) {
                this.transactions = this.transactions.filter(
                  (t) =>
                    t.store?._id === this.storeId ||
                    t.toStore?._id === this.storeId
                );
              }
  ...
  toggleFlag(transaction: Transaction) {
    ...
    if (transaction.issue) {
      this.transactionsService.unflagTransaction(transaction._id).subscribe(
        (res: any) => {
          this.updateTransactions();
        },
        (e) => {
