...
export class UsersComponent implements OnInit, OnDestroy {
  ...
  saveUser(dialog: any) {
    ...
    this.userService.updateUser(this.user._id, this.userForm.value).subscribe(
      ...
      (e) => {
        console.error(e);
        this.msg.error(
          'Sorry, could not update user at the moment. Please try again later.',
          'Error'
        );
      },
      () => {
  ...
  initUserForm() {
    ...
    this.userForm = this.fb.group({
      ...
      telegramId: [
        this.user.telegram && this.user.telegram.id,
        [Validators.pattern(/[0-9]+/)],
      ],
      telegramName: [
        !!this.user.telegram && !!this.user.telegram.name
          ? this.user.telegram.name
          : '',
      ],
    });
  }
