...
export class BtcDashboardComponent implements OnInit {
    ...
    this.transactionsService
      ...
      .subscribe(
        (res: any) => {
          ...
          this.btcForm
            .get("conversion_fee")
            ?.setValue(this.defaultConversionFee);
          this.otherForm
            .get("conversion_fee")
            ?.setValue(this.defaultConversionFee);
  ...
  makeBtc() {
    ...
    req.conversion_fee =
      (req.conversion_fee / 100) * (req.crypto_ammount * req.price_current);
    req.service_fee =
      (req.service_fee / 100) * (req.crypto_ammount * req.price_current);
    ...
    this.transactionsService.makeCryptoSale(req).subscribe(
      (res: any) => {
        ...
        if (res.data.makeCryptoSale) {
          var latestPostId =
            res.data.makeCryptoSale.posts[
              res.data.makeCryptoSale.posts.length - 1
            ]._id;
  ...
  onClientChange(postId: string, eventTarget: any) {
    ...
    this.transactionsService.updatePostClient(postId, newClient).subscribe(
      ...
      (e) => {
        console.error(e);
        this.msg.error(
          "Could not update client for this transaction...",
          "Error!"
        );
      }
    );
